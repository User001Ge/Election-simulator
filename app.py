...
def _candidate_options_for_display(candidate_options: list[str]) -> list[str]:
    ...
    target_index = next(
        (i for i, name in enumerate(ordered) if "მელქისედეკ" in name),
        None,
    )
...
def _render_static_vote_chart(vote_df: pd.DataFrame) -> None:
    ...
    ax.set_xlabel("")
    ax.set_title("ხმების განაწილება ერთ კონკრეტულ გაშვებაში", fontweight="bold", pad=14)
...
def _render_probability_chart(probability_df: pd.DataFrame) -> None:
    ...
    ax.set_xlabel("")
    ax.set_title("Monte Carlo-ს შედეგები", fontweight="bold", pad=14)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.grid(axis="y", linestyle="--", alpha=0.25)
    ax.set_axisbelow(True)
...
def _display_probability_metrics(monte_carlo_result: dict) -> None:
    ...
    with left:
        st.metric(
            monte_carlo_result["selected_candidates"][0],
            f'{monte_carlo_result["winner_probabilities"][monte_carlo_result["selected_candidates"][0]]:.1%}',
        )
    with middle:
        st.metric(
            monte_carlo_result["selected_candidates"][1],
            f'{monte_carlo_result["winner_probabilities"][monte_carlo_result["selected_candidates"][1]]:.1%}',
        )
    with right:
        st.metric(
            monte_carlo_result["selected_candidates"][2],
            f'{monte_carlo_result["winner_probabilities"][monte_carlo_result["selected_candidates"][2]]:.1%}',
        )
    with far_right:
...
def main() -> None:
    ...
    if "candidate_1" not in st.session_state or st.session_state["candidate_1"] not in display_candidate_options:
        st.session_state["candidate_1"] = default_candidates[0]
    if "candidate_2" not in st.session_state or st.session_state["candidate_2"] not in display_candidate_options:
        st.session_state["candidate_2"] = default_candidates[1]
    if "candidate_3" not in st.session_state or st.session_state["candidate_3"] not in display_candidate_options:
        st.session_state["candidate_3"] = default_candidates[2]
    ...
    with st.sidebar:
        ...
        if candidate_2_default not in candidate_2_options:
            candidate_2_default = (
                default_candidates[1] if default_candidates[1] in candidate_2_options else candidate_2_options[0]
            )
            st.session_state["candidate_2"] = candidate_2_default
        ...
        candidate_3_options = [
            name for name in display_candidate_options if name not in {candidate_1, candidate_2}
        ]
        ...
        if candidate_3_default not in candidate_3_options:
            candidate_3_default = (
                default_candidates[2] if default_candidates[2] in candidate_3_options else candidate_3_options[0]
            )
            st.session_state["candidate_3"] = candidate_3_default
        ...
        iterations = st.number_input(
            "Monte Carlo ინტერაციები",
            min_value=1,
            max_value=100_000,
            value=int(default_params["default_iterations"]),
            step=100,
        )
        volatility_level = st.number_input(
            "რყევადობის დონე",
            min_value=0,
            max_value=20,
            value=int(default_params["volatility_level"]),
            step=1,
        )

        voter_absence_percent = st.slider(
            "ამომრჩევლის გაცდენის შანსი",
            min_value=0,
            max_value=100,
            value=int(round(float(default_params["voter_absence_probability"]) * 100)),
            step=1,
            format="%d%%",
        )
        candidate_absence_percent = st.slider(
            "კანდიდატის გაცდენის შანსი",
            min_value=0,
            max_value=100,
            value=int(round(float(default_params["candidate_absence_probability"]) * 100)),
            step=1,
            format="%d%%",
        )
        ...
        enable_voter_absence = st.checkbox(
            "გააქტიურდეს ამომრჩევლის გაცდენა",
            value=bool(default_params["enable_voter_absence"]),
        )
        enable_candidate_absence = st.checkbox(
            "გააქტიურდეს კანდიდატის გაცდენა",
            value=bool(default_params["enable_candidate_absence"]),
        )
        show_full_single_run = st.checkbox(
            "ერთი გაშვების სრული ცხრილის ჩვენება",
            value=False,
        )
        show_preferences_matrix = st.checkbox(
            "პრეფერენციების მატრიცის ჩვენება",
            value=False,
        )
        seed_text = st.text_input("Seed (არასავალდებულო)", value="")
        run_button = st.button("სიმულაციის გაშვება", type="primary", use_container_width=True)
    ...
    if mc is None or single is None:
        st.info(
            "აპი პირველივე გახსნაზე აღარ უშვებს ავტომატურ მძიმე გამოთვლას. აირჩიე პარამეტრები და დააჭირე **სიმულაციის გაშვება**."
        )
        return
    ...
    single_left, single_right = st.columns([0.9, 1.5])
    with single_left:
        st.dataframe(single_vote_totals_df, use_container_width=True, hide_index=True)
    with single_right:
        _render_static_vote_chart(single_vote_totals_df)

    if show_full_single_run:
    ...
    with col1:
        ...
        with st.expander("ალბათობების ცხრილი", expanded=False):
            st.dataframe(
    ...
    with col2:
        ...
        st.dataframe(vote_stats_df, use_container_width=True, hide_index=True)
        st.info(
            f"საშუალოდ ესწრება {mc['average_present_count']:.2f} მღვდელმთავარი, "
            f"ხოლო აცდენს {mc['average_absent_count']:.2f}."
        )

    if show_preferences_matrix:
