...
def main() -> None:
    ...
    if run_button or st.session_state["last_monte_carlo"] is None:
        ...
        single_run_result = simulate_single_election(
            model=MODEL,
            selected_candidates=selected_candidates,
            voter_absence_probability=float(voter_absence_probability),
            volatility_level=int(volatility_level),
    ...
    # ამოვიღოთ გამარჯვებული
    winner_name = single['winner']
    
    # შევამოწმოთ, ეს კონკრეტული სახელია თუ "მეორე ტური"
    # RUNOFF_LABEL იმპორტირებული გაქვთ ფაილის დასაწყისში
    if winner_name == RUNOFF_LABEL:
        st.warning(f"### 🎰 შედეგი: **{winner_name}**", icon="⚠️")
    else:
        st.success(f"### 🏆 გამარჯვებული: **{winner_name}**", icon="🎉")

    # სტატისტიკა ცალკე, პატარა ტექსტით
    ...
    with st.expander("პრეფერენციების მატრიცა (Excel-ის N:V ბლოკის ანალოგი)"):
